"""
ARS implementation for learning the swimmer task.
RL Agent implementation in the RL Glue framework.
"""
...
class SwimmerARSAgent(Agent):
  ...
  def agent_init(self, taskSpec):
    ...
    self.deltas = [
        2 * np.zeros((self.n_action, self.n_obs)) - 1 for i in range(self.N)]
    self.deltaPolicies = [self.agentPolicy for i in range(
        2 * self.N)]  # 2N policies for the 2N rollouts
  ...
  def agent_start(self, observation):
    ...
    # Fix the observation at the begining of the iteration
    ...
    # Select action given the policy and the observation
    thisAction = self.select_action(thisPolicy, observation)
    print("Agent started!")
  ...
  def agent_step(self, reward, observation):
    assert len(observation.doubleArray) == self.n_obs
    # In general case, the observation used to select the action is the last
    # observation given by the environment
    thisObservation = copy.deepcopy(observation)
    ...
    if not self.freeze:
      ...
      if self.count % self.H == 0:
        ...
        # Update reward. The current reward is the one received at the end of
        # the previous rollout.
        self.rewards[idx] = self.total_reward
        self.total_reward = 0.
        # At the begining of a new rollout, we start again with the initial
        # observation.
        thisObservation = copy.deepcopy(self.initial_state)
    ...
    # Select action given the policy and the observation
    thisAction = self.select_action(thisPolicy, thisObservation)
    ...
    if not self.freeze:
      ...
      if self.count % (2 * self.N * self.H) == 0:
        ...
        # Use the previous rewards to update the policy. Only after the first
        # iteration.
        self.update_policy(order)
        self.sample_deltas()  # Choose the deltas directions
        for i in range(len(self.rewards)):
          self.rewards[i] = 0.  # Reset rewards
  ...
  def agent_message(self, inMessageByte):
    ...
    if inMessage == "set parameters":
      self.set_parameters()
      s = f"Agent parameters are: N={self.N}; b={self.b}, "
      s += f"H={self.H}, alpha={self.alpha}, nu={self.nu}"
      print(s)
      return s
  ...
  def sample_deltas(self):
    ...
    for i in range(len(self.deltas)):
      self.deltas[i] = np.random.rand(self.agentPolicy.shape[0],
                                      self.agentPolicy.shape[1])
      self.deltaPolicies[2 * i] = self.agentPolicy + self.nu * self.deltas[i]
      self.deltaPolicies[2 * i + 1] = \
          self.agentPolicy - self.nu * self.deltas[i]

  def order_directions(self):
    """
    Sort the directions delta_k by max(reward[k,+], reward[k,-]).
    Return the array of indices corresponding to ordering from largest to
    smallest direction.
    """
    # TODO
  ...
  def update_policy(self, order):
    ...
    for i in range(self.b):
      used_rewards += [self.rewards[2 * order[i]],
                       self.rewards[2 * order[i] + 1]]
    sigma_r = stdev(used_rewards)

    grad = np.zeros(self.agentPolicy.shape)
    for i in range(self.b):
      grad += (self.rewards[2 * order[i]] -
               self.rewards[2 * order[i] + 1]) * self.deltas[order[i]]
    grad /= (self.b * sigma_r)
