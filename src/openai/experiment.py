...
class Experiment():
  ...
  def plot(self, n_seed, agent_param):
    ...
    for rewards in r_graphs:
      plt.plot(t, rewards)
    if agent_param.safe:
      plt.plot(t, [agent_param.threshold] * len(t), label="Safety threshold")
      plt.legend()
...
if __name__ == '__main__':
  ...
  hand_exp = Experiment(real_env_param,
                       ...
                       guess_param=None)
  mean_returns = hand_exp.plot(n_seed=4, agent_param=hand_agent)
  ...
  real_agent = ARSParam('RLControl', V1=True, n_iter=100, H=1000, N=1, b=1,
