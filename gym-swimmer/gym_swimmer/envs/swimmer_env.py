...
class SwimmerEnv(gym.Env):
  ...
  def __init__(self):
    ...
    self.action_space = spaces.Box(-self.max_u,
                                   self.max_u, shape=(n_action,),
                                   dtype=np.float32)
  ...
  def compute_accelerations(self, torque, G_dot, theta, theta_dot):
    ...
    for i in range(1, self.n + 1):
      ...
      A[i - 1, self.n + 2 * (i - 1) + 0] = +self.l_i / \
          2. * np.sin(theta[i - 1])
      A[i - 1, self.n + 2 * (i - 1) + 2] = +self.l_i / \
          2. * np.sin(theta[i - 1])
      A[i - 1, self.n + 2 * (i - 1) + 1] = -self.l_i / \
          2. * np.cos(theta[i - 1])
      A[i - 1, self.n + 2 * (i - 1) + 3] = -self.l_i / \
          2. * np.cos(theta[i - 1])
    ...
    for i in range(1, self.n):
      for d in range(2):
        ...
        if d == 0:
          A[3 * self.n + 4 + 2 * (i - 1) + d][i - 1] = - \
              self.l_i / 2. * np.sin(theta[i - 1])
          A[3 * self.n + 4 + 2 * (i - 1) + d][i] = - \
              self.l_i / 2 * np.sin(theta[i])
          B[3 * self.n + 4 + 2 * (i - 1) + d] = +self.l_i / 2. * (
              np.cos(theta[i - 1]) * theta_dot[i - 1] ** 2 +
              np.cos(theta[i]) * theta_dot[i] ** 2)

        else:
          A[3 * self.n + 4 + 2 * (i - 1) + d][i - 1] = + \
              self.l_i / 2. * np.cos(theta[i - 1])
          A[3 * self.n + 4 + 2 * (i - 1) + d][i] = + \
              self.l_i / 2. * np.cos(theta[i])
          B[3 * self.n + 4 + 2 * (i - 1) + d] = +self.l_i / 2. * (
              np.sin(theta[i - 1]) * theta_dot[i - 1] ** 2 +
              np.sin(theta[i]) * theta_dot[i] ** 2)
  ...
  def compute_friction(self, G_dot, theta, theta_dot):
    """
    Compute frictions for constructing the matrix used
    in compute_accelerations().
    ...
    normal = np.array([np.array([-np.sin(theta[i]), np.cos(theta[i])])
    ...
    for i in range(self.n):
      if i == 0:
        ...
        M[i][i - 1] = -1
        vecX[i] = self.l_i / 2 * \
            (theta_dot[i - 1] * normal[i - 1][0] + theta_dot[i] * normal[i][0])
        vecY[i] = self.l_i / 2 * \
            (theta_dot[i - 1] * normal[i - 1][1] + theta_dot[i] * normal[i][1])
    # print(f"theta = {theta}")
    # print(f"M = {M}")
    # print(f"vecX = {vecX}")
    ...
    F_friction = [-self.k * self.l_i *
                  np.dot(G_i_dot[i], normal[i]) * normal[i]
                  for i in range(self.n)]
    M_friction = [-self.k * theta_dot[i] *
                  self.l_i ** 3 / 12. for i in range(self.n)]
...
if __name__ == '__main__':
  ...
  G_dotdot, theta_dotdot = env.compute_accelerations(
      action, G_dot, theta, theta_dot)
  print(f"Accelerations: G_dotdot = {G_dotdot}, theta_dotdot = {theta_dotdot}")
  G_dot, theta, theta_dot = env.next_observation(
      action, G_dot, theta, theta_dot)
  print(G_dot, theta, theta_dot)
